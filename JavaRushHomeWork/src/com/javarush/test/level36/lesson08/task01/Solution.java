...
public class Solution {
    public static void main(String[] args) throws IOException
    {
        ...
        try (FileReader fr = new FileReader(filename))
        {
            ...
            while (fr.ready())
            {
                String temp = String.valueOf((char) fr.read()).toLowerCase();

                if (temp.matches("\\w") && temp.matches("\\D") && !temp.matches("_"))
                {
                    treeSet.add(temp.charAt(0));
                }
            }
        }
        for (int i = 0; i < 5; i++)
        {
            String temp = String.valueOf(treeSet.pollFirst());

            if (!temp.equals("null")) System.out.print(temp);
        }
    }
}
